...
def run_benchmark(*, items: int, repeats: int, limit: int) -> dict[str, object]:
    ...
    if optimized_prepared != legacy_prepared:
        raise AssertionError("streamed full pool changed cold prepared rows")
    ...
    if optimized_results != legacy_results:
        raise AssertionError("streamed full pool changed cold recommendation output")

    legacy_prepare = _summary(
        _timed(lambda: _legacy_prepare(engine, user_id), repeats=repeats)
    )
    ...
    legacy_recommend = _summary(
        _timed(
            lambda: engine.rank_prepared(
                _legacy_prepare(engine, user_id),
                limit=limit,
            ),
            repeats=repeats,
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Benchmark streamed cold recommendation full pool."
    )
    ...
    if failures:
        raise SystemExit(
            "recommend full-pool performance guardrail failed: "
            + "; ".join(failures)
        )
