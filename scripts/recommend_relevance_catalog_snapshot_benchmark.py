...
def _fixture(*, items: int, users: int) -> tuple[Catalog, RecommendationEngine]:
    rows = [
        Item(
            ...
            text="recommend relevance validated catalog snapshot benchmark",
...
def run_benchmark(*, items: int, users: int, repeats: int) -> dict[str, object]:
    ...
    def legacy_catalog(
        ...
        *,
        ...
        return Catalog(
            ...
            query_labels=list(current.query_labels),
            events=[],
            reward_spec=None,
            name=f"{current.name}:temporal-relevance:{user_id}",
        )

    def legacy_prepare():
        validation._temporal_training_catalog = legacy_catalog
        try:
            ...
            validation._temporal_training_catalog = optimized_helper
    ...
    if optimized.evaluate(engine.config) != legacy.evaluate(engine.config):
        raise AssertionError("validated catalog snapshot changed relevance evaluation")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Benchmark reused validated temporal catalog snapshots."
    )
    ...
    if failures:
        raise SystemExit(
            "recommend relevance catalog snapshot performance guardrail failed: "
            + "; ".join(failures)
        )
