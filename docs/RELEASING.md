...
7. release assets include SHA-256 checksums.
...
The Release workflow supports `workflow_dispatch`. A manual run executes the build-and-verify job and uploads the verified bundle as a workflow artifact, but it **does not create a GitHub Release** because no immutable release tag is present.
...
The tag starts the Release workflow. Its read-only build job runs the release acceptance suite, builds the wheel and source distribution, verifies clean installs, generates `SHA256SUMS`, and uploads the bundle. Only after that job succeeds does the separate publish job receive `contents: write`; the publish job executes no repository code and creates the immutable GitHub Release from the already-verified artifacts.
