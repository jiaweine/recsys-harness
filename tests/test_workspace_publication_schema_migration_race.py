...
class _BarrierConnection:
    ...
    def execute(self, sql: str, *args, **kwargs):
        ...
        if sql.strip().lower() == "pragma table_info(workspace_state)":
...
def test_concurrent_first_upgrade_installs_publication_column_once(tmp_path, monkeypatch):
    path = tmp_path / "workspace-publication-schema-race.db"
    ...
    with sqlite3.connect(path) as connection:
        columns = [row[1] for row in connection.execute("pragma table_info(workspace_state)")]
    assert columns.count("publication_revision") == 1
