...
def test_hot_config_cannot_evict_current_routing_prior_pairs(tmp_path, monkeypatch):
    ...
    # 192-row history by exactly four rows. Pure global-recency retention evicts the
    # four t=1000 prior rows above and turns a real 4/4 feasibility flip into a false
    # negative. Pair-floor retention must keep those paid priors without capping the
    # hot config's repeated-evidence history to two rows.
    clock["now"] = 3_000.0
    ...
    assert (
        ...
        == "current_routing_pair_floor_then_global_recency"
    )
