...
def audit_search(catalog: Catalog, engine: SearchEngine) -> dict:
    labels = _stable_sample(list(catalog.query_labels), MAX_AUDIT_QUERIES, lambda x: x.query)
    rows = []
    for label in labels:
    ...
    return {
        "queries": len(rows),
        "available_queries": len(catalog.query_labels),
        "sampled": len(rows) < len(catalog.query_labels),
