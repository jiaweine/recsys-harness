...
class SearchEngine:
    ...
    def search(self, query: str, *, limit: int = 10) -> list[dict]:
        ...
        for item in self.catalog.items:
            ...
            title_tokens = set(tokenize(item.title)); overlap = len(set(qtokens)&title_tokens)/max(1, len(set(qtokens)))
            exact = 1.0 if query.lower() in item.title.lower() else 0.0
            sem = max(0.0, cosine(qvec, self._vectors[item.item_id]))
            rows.append({"item": item, "lex": lex, "sem": sem, "title": min(1.0, overlap*.7+exact*.55)})
        for row in rows:
        ...
        while pool and len(selected) < limit:
            ...
            for row in pool:
                redundancy = max((self._cat_similarity(row["item"], s["item"]) for s in selected), default=0.0)
