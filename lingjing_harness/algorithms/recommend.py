...
class RecommendConfig:
    ...
    cold_start: float = field(default=0.06, metadata=_BLEND)
...
class RecommendationEngine:
    ...
    def __init__(self, catalog: Catalog, config: RecommendConfig | None = None) -> None:
        ...
        self._vectors = {
            x.item_id: hashed_vector(" ".join([x.title, x.text, *x.categories]))
    ...
    def _profile_with_decay(
        ...
        *,
        ...
        seen = {e.item_id for e in events}
    ...
    def prepare(self, user_id: str) -> list[dict]:
        ...
        for item_id in candidate_ids:
            ...
            pop = self._popularity[item.item_id]
            novelty = 1.0 - pop
...
def _candidate_evidence_union(
    ...
    if cats:
        for item in eligible:
            if set(item.categories) & set(cats):
...
def _category_similarity(engine: RecommendationEngine, a: Item, b: Item) -> float:
    aa, bb = set(a.categories), set(b.categories)
    return len(aa & bb) / max(1, len(aa | bb))


def _semantic_similarity(engine: RecommendationEngine, a: Item, b: Item) -> float:
    return max(0.0, cosine(engine._vectors[a.item_id], engine._vectors[b.item_id]))


def _hybrid_similarity(engine: RecommendationEngine, a: Item, b: Item) -> float:
