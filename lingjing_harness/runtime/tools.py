...
class ToolRegistry:
    """Capability registry with risk classes, guards and persistent strategy memory."""
    ...
    def _build_specs(self) -> dict[str, ToolSpec]:
        specs = {
            ...
            "search.run": ToolSpec(
                "search.run", "Run the current search experience", "read", self.run_search,
                cost=.9, input_schema={"type": "object", "properties": {"query": {"type": "string", "maxLength": 50}}},
            ),
            "search.diagnose": ToolSpec(
                "search.diagnose", "Diagnose query evidence and candidate coverage", "read", self.search_diagnose,
                cost=.7, input_schema={"type": "object", "properties": {"query": {"type": "string", "maxLength": 50}}},
            ),
            "search.audit": ToolSpec(
                "search.audit", "Evaluate search on labeled queries", "simulation", self.search_audit,
                cost=2.0, input_schema={"type": "object", "properties": {}},
            ),
            "search.evolve": ToolSpec(
                "search.evolve", "Generate and robustly evaluate evolved search strategies", "adaptive", self.search_evolve,
                cost=6.0, side_effect="internal_strategy_memory",
                input_schema={"type": "object", "properties": {"activate": {"type": "boolean"}}},
            ),
            "recommend.run": ToolSpec(
                "recommend.run", "Generate a recommendation slate", "read", self.run_recommend,
                cost=1.0, input_schema={"type": "object", "properties": {"user_id": {"type": "string", "maxLength": 120}}},
            ),
            "recommend.diagnose": ToolSpec(
                "recommend.diagnose", "Diagnose user evidence, cold start and eligible pool", "read", self.recommend_diagnose,
                cost=.7, input_schema={"type": "object", "properties": {"user_id": {"type": "string", "maxLength": 120}}},
            ),
            "recommend.audit": ToolSpec(
                "recommend.audit", "Evaluate recommendation coverage, freshness and diversity", "simulation", self.recommend_audit,
                cost=2.2, input_schema={"type": "object", "properties": {}},
            ),
            "recommend.evolve": ToolSpec(
                "recommend.evolve", "Generate and robustly evaluate evolved recommendation strategies", "adaptive", self.recommend_evolve,
                cost=6.5, side_effect="internal_strategy_memory",
                input_schema={"type": "object", "properties": {"activate": {"type": "boolean"}}},
            ),
        }
    ...
    def _validate_active_strategies(self) -> None:
        ...
        if search_skill and not self._validation_is_fresh(search_skill):
            ...
            if regression:
                retired = self.memory.retire_active(self.catalog_key, "search", reason="active strategy regressed against the owned default")
                self.search = default_engine
                if retired:
                    self.rollback_events.append({"domain": "search", **retired})
            else:
                self.memory.mark_skill_validation(
                    self.catalog_key, "search", search_skill["fingerprint"],
                    metrics={"quality": active_report.get("quality", 0.0), "recall": active_report.get("recall", 0.0)},
                )
        ...
        if recommend_skill and not self._validation_is_fresh(recommend_skill):
            ...
            if regression:
                retired = self.memory.retire_active(self.catalog_key, "recommend", reason="active strategy regressed against the owned default")
                self.recommend = default_engine
                if retired:
                    self.rollback_events.append({"domain": "recommend", **retired})
            else:
                self.memory.mark_skill_validation(
                    self.catalog_key, "recommend", recommend_skill["fingerprint"],
                    metrics={"quality": active_report.get("quality", 0.0), "coverage": active_report.get("coverage", 0.0)},
                )
    ...
    def inspect_data(self) -> dict[str, Any]:
        ...
        duplicates = len(self.catalog.items) - len({x.title.strip().lower() for x in self.catalog.items})
        if duplicates:
    ...
    def search_diagnose(self, query: str | None = None, **_: Any) -> dict[str, Any]:
        ...
        covered = set()
        for row in result:
            title = next((item.title for item in self.catalog.items if item.item_id == row["id"]), row["title"])
        ...
        return {
            ...
            "diagnosis": (
                "没有候选包含可验证的查询词证据" if not result
                else "查询包含较宽泛词，排序需要更多依赖具体词证据" if generic
                else "当前候选存在但首位匹配证据偏弱" if result[0]["signals"]["match"] < .42
    ...
    def search_evolve(self, activate: bool = False, _invocation_id: str | None = None, **_: Any) -> dict[str, Any]:
        if _invocation_id:
            replay = self.memory.invocation_result(_invocation_id)
            if replay:
                result = dict(replay["result"])
                result.update({"skill": replay["skill"], "learned": True, "activated": replay["skill"]["status"] == "active", "replayed": True})
                if result["activated"]:
                    self.search = self.search.with_config(SearchConfig(**result["candidate_config"]))
                return result
        ...
        if result.get("trusted"):
            ...
            skill = self.memory.remember_strategy(
                ...
                payload={
                    ...
                    **({"validated_at": time.time(), "validation": result.get("candidate", {})} if activate else {}),
                },
            ...
            result["learned"] = True
            if activate:
                self.search = SearchEngine(self.catalog, SearchConfig(**result["candidate_config"]))
    ...
    def recommend_diagnose(self, user_id: str | None = None, **_: Any) -> dict[str, Any]:
        ...
        eligible = [item for item in self.catalog.items if item.eligible and item.item_id not in seen]
        cats = sorted({category for event in events for category in self.catalog.item_by_id[event.item_id].categories})
        return {
            "user_id": user_id,
            "history_events": len(events),
            "seen_items": len(seen),
            "eligible_unseen": len(eligible),
            "known_categories": cats[:12],
            "cold_start": len(events) == 0,
            "diagnosis": (
                "这是冷启动用户，当前结果主要依赖内容质量、新鲜度和稳定探索" if not events
                else "可展示未看内容不足，推荐空间受到候选池限制" if len(eligible) < 8
    ...
    def recommend_evolve(self, activate: bool = False, _invocation_id: str | None = None, **_: Any) -> dict[str, Any]:
        if _invocation_id:
            replay = self.memory.invocation_result(_invocation_id)
            if replay:
                result = dict(replay["result"])
                result.update({"skill": replay["skill"], "learned": True, "activated": replay["skill"]["status"] == "active", "replayed": True})
                if result["activated"]:
                    self.recommend = self.recommend.with_config(RecommendConfig(**result["candidate_config"]))
                return result
        ...
        if result.get("trusted"):
            ...
            result["learned"] = True
            if activate:
                self.recommend = RecommendationEngine(self.catalog, RecommendConfig(**result["candidate_config"]))
                result["activated"] = True
        return result
