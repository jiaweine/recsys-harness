    ...
    def run(self,text:str,*,sink:EventSink|None=None)->dict[str,Any]:
        ...
        for i,step in enumerate(plan.steps[:self.max_tools]):
            ...
            if step.tool=="search.run":
                ...
                if result.get("queries"): blocks.append(f"我又用 {result['queries']} 个已知查询做了复核，整体命中质量约为 {result['quality']:.0%}。")
                ...
                blocks.append(f"当前内容覆盖约 {result.get('coverage',0):.0%}，新鲜度约 {result.get('freshness',0):.0%}，结果分散度约 {result.get('diversity',0):.0%}。")
            elif step.tool=="recommend.compare":
                findings.extend(self.verifier.experiment(result)); d=result["delta"]
                if result.get("safe_to_try"): blocks.append(f"候选方案通过离线门槛，新鲜度变化 {d.get('freshness',0):+.1%}，覆盖变化 {d.get('coverage',0):+.1%}。")
                else: blocks.append("候选推荐方案存在覆盖或整体质量回退，先不建议扩大流量。")
